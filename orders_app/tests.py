...
def offer_detail(db, business_user):
    ...
    detail = OfferDetail.objects.create(
        ...
        price=150,
        features=["Logo Design", "Business Cards"],
    )
    return detail
...
@pytest.mark.django_db
def test_orders_list_returns_only_related(
    api_client,
    customer_user,
    business_user,
    offer_detail,
    ...
    ids = [order["id"] for order in res.data]
    assert order1.id in ids
    assert len(ids) == 1


@pytest.mark.django_db
def test_business_can_patch_status_only_own_order(
    api_client,
    customer_user,
    business_user,
    offer_detail,
...
@pytest.mark.django_db
def test_customer_cannot_patch_order(
    api_client,
    customer_user,
    business_user,
    offer_detail,
...
@pytest.mark.django_db
def test_order_count_endpoints(
    api_client,
    customer_user,
    business_user,
    offer_detail,
